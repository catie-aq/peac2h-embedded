...
export default function Home() {
  ...
  if (error || allSubjectsError) return <div>échec du chargement</div>
  if (isLoading || allSubjectsLoading) return <div>chargement...</div>
                      ...
                      return (
                        ...
                        <div key={t_idx} className='flex flex-col gap-4 items-center'>
                          ...
                          { subjects.map((subject, s_idx) => {
                            const inProgress = subject.hasOwnProperty("partial-S" + t_idx)
                            const finished = !subject["partial-S" + t_idx] && subject["partial-S" + t_idx] != undefined
                            ...
                            return (
                              <h3 key={s_idx} style={{ color: textColor }}>{subject["name"]}</h3>
                            )
                          })}
                        </div>
                ...
                return (
                  <div key={g_idx}>
                    <Group group={group} g_idx={g_idx} studyId={studyId} 
                          subjects={subjects} allSubjects={allSubjects} mutate={mutateAllSubjects}
                          showGroup={true}/>
                  </div>
                )
              }else {
